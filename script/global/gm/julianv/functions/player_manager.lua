...
function JulianV:NhanDanhHieu()
    local tbOpt = {
        ...
        { "Danh hiÖu KiÕm ThÕ", JulianV.DanhHieuKiemThe }, --
    }
    JDialog:Show(tbOpt, JulianV.Player_Dialog)
end
...
function JulianV.DanhHieuKiemThe( nIndex )
    if not nIndex then
        ...
        Msg2Player("KÝch ho¹t danh hiÖu <color=yellow>" .. tbDanhHieu.KiemThe[nIndex][1])
    end
end
...
function JulianV.DanhHieuVoLam( nPage )
    ...
    if not nPage or nPage == 1 then
        ...
        if nPage > 1 then
            tinsert(tbOpt, { "Trang tr­íc", JulianV.DanhHieuVoLam, { nPage - 1 } })
        end
    end
    JDialog:Show(tbOpt, nil, "Mêi <sex>chän danh hiÖu<pic=46><color>")
end
