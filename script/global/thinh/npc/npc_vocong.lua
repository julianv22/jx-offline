...
function dialog_vocong_main()
    ...
    local tbOpt = {"Vµo Ph¸i Vµ Häc Skill 180/choose_faction", "Häc Khinh C«ng/khinhcong",
                   "NhËn c¸c lo¹i ®iÓm/CacLoaiDiem", "LÊy Skill Theo ID/LaySkillID",
                   "§æi Tªn Nh©n VËt/doiten", "ChuyÓn ®æi giíi tÝnh, Ngò Hµnh/nguhanh",
                   "TÈy Tñy/clear_attibute_point", "KÕt thóc ®èi tho¹i./no"}
...
function nguhanh()
    ...
    local tbOpt = {"ChuyÓn ®æi N÷ sang hÖ Kim/nukim", "Chuyªn ®æi Nam sang hÖ Thñy/namthuy",
                   "ChuyÓn Giíi/chuyengioi", "Trë l¹i/dialog_vocong_main", "KÕt thóc ®èi tho¹i./no"}
    Say(szTitle, getn(tbOpt), tbOpt)
end
...
function choose_faction()
    ...
    local szTitle = "Xin chµo <color=red>" .. GetName() ..
                        "<color>. Mét khi gia nhËp m«n ph¸i kh«ng thÓ thay ®æi, h·y suy nghÜ kü"
    local tbOpt = {}
...
function vuhon_hoason(num)
    ...
    if num == 0 then
        szTitle = "<color=red>" .. GetName() ..
                      "<color> ng­¬i cã ch¾c ch¾n muèn gia nhËp ph¸i <color=yellow>Hoa S¬n<color> kh«ng?"
        tbOpt = {"X¸c nhËn!/vaohoason", "Trë VÒ/dialog_vocong_main", "Kªt thóc ®èi tho¹i./no"}
    elseif num == 1 then
        szTitle = "<color=red>" .. GetName() ..
                      "<color> ng­¬i cã ch¾c ch¾n muèn gia nhËp ph¸i <color=yellow>Vò Hån<color> kh«ng?"
        tbOpt = {"X¸c nhËn!/vaovuhon", "Trë VÒ/dialog_vocong_main", "Kªt thóc ®èi tho¹i./no"}
    elseif num == 2 then
        szTitle = "<color=red>" .. GetName() ..
                      "<color> ng­¬i cã ch¾c ch¾n muèn gia nhËp ph¸i <color=yellow>Tiªu Dao<color> kh«ng?"
        tbOpt = {"X¸c nhËn!/vaotieudao", "Trë VÒ/dialog_vocong_main", "Kªt thóc ®èi tho¹i./no"}
...
function set_faction(nIndex)
    local szTitle = format("<color=red>" .. GetName() ..
                               "<color> ng­¬i cã ch¾c ch¾n muèn gia nhËp ph¸i <color=yellow>%s<color> kh«ng?",
        tbFaction[nIndex].szShowName)
    local tbOpt = {"X¸c nhËn!/#do_set_faction(" .. nIndex .. ")", "Trë VÒ/choose_faction",
...
function do_set_faction(nIndex)
    ...
    SetCamp(4)
    Talk(1, "KickOutSelf",
        format("Ng­¬i ®· gia nhËp thµnh c«ng ph¸i <color=yellow>%s", tbFaction[nIndex].szShowName))
end
...
function clear_attibute_point()
    ...
    local tbOpt = {"TÈy §iÓm Kü N¨ng./do_clear_skill", "TÈy §iÓm TiÒm N¨ng./do_clear_prop",
                   "Trë VÒ/dialog_vocong_main", "Tho¸t./no"}
    Say(szTitle, getn(tbOpt), tbOpt)
end
...
function LaySkillID()
    ...
    local tbOpt = {{"LÊy Skill", g_AskClientStringEx, {"69,20", 0, 256, "ID Skill", {AddSkill, {self}}}},
                   ...
                   {"Trë vÒ", dialog_vocong_main}, {"Tho¸t."}}
