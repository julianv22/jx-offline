...
function TaskShedule()
	-- ÉèÖÃ·½°¸Ãû³Æ
	TaskName( "MAKE BIG BOSS 23:15" );
	TaskInterval( 1440 );
	TaskTime( 23, 15 );
	TaskCountLimit( 0 );
	-- Êä³öÆô¶¯ÏûÏ¢
	OutputMsg( "MAKE BIG BOSS AT :  23:15" );
...
function TaskContent()
	...
	if (BigBoss.gold_boss_count == 0) then
		...
		local str = format("Giang hå truyÒn r»ng tuyÖt thÕ cao thñ <color=yellow>%s<color>  ®· xuÊt hiÖn ë <color=yellow>%s<color> ! Giang hå ¾t sÏ cã mét trËn ®Ém m¸u!", bossname, worldname);
